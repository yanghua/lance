...
| 256      | `FLAG_CLUSTERING_VERSION`       | No              | Yes             | The dataset has an active liquid-clustering configuration or fragments with clustering layout generation/group stamps. Older writers must reject it because historical rewrite paths did not preserve this metadata; readers may ignore the physical-layout metadata. |
