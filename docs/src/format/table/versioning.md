...
| 2048     | `FLAG_CLUSTERING_METADATA`      | No              | Yes             | The dataset carries [liquid-clustering](liquid_clustering.md) configuration or fragment layout stamps. Older writers must reject it because rewrites could silently discard the metadata; readers may ignore physical-layout metadata. |
